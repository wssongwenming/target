...
@Service
public class CameraMarkDataConsumer3 implements ChannelAwareMessageListener {
    ...
    public CameraMarkDataConsumer3(){
    }
    /**
     * 判断点是否在多边形内
     * @param point 检测点
     ...
     * @return
     */
    private double ringNumber(double px,double py,double r){
        ...
        try {
            ...
            if(traineeIdsInShooting!=null) {
                ...
                for(int i=0;i<scoreCountInJson;i++){
                    ...
                    Double px=newScore.getDouble("px");//得到识靶终端的px，py，这时数据是以靶心为坐标原点的，所以为了完成各项检
                    // 测需要转换为屏幕坐标，以左上角为坐标原点
                    Double py=newScore.getDouble("py");
                    //将以靶心为原点的坐标转换为了以左上角为参考点的坐标,
                    Double PX=px+bullEyePoint.getX();
                    Double PY=py+bullEyePoint.getY();
                    Point2D point =new Point2D(PX,PY);

                    if(isInvalidArea(pts,firstConjunction,secondConjunction,centerPoint,r,point)) {
                        ...
                        Double ringNumber=ringNumber(px,py,radius);//radius应该是39.8,如果不是和前面的数据会有问题
                        scoresParam.setRingnumber(Float.valueOf(String.valueOf(ringNumber)));
                        scoresParam.setScoreIndex(i);
